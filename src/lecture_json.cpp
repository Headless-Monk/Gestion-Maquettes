...
maquette& lecture_json::lire_une_maquette()
{
    ...
    while(typeUE != "}")
    {
        ...
        for(unsigned int i = 0; i < tmpUEComposeeMaquette.size(); i++)
            maquetteCourante.ajouter_ue(tmpUEComposeeMaquette[i]);
        for(unsigned int i = 0; i < tmpUESeulesMaquette.size(); i++)
            maquetteCourante.ajouter_ue(tmpUESeulesMaquette[i]);
        for(unsigned int i = 0; i < tmpUEChoixMaquette.size();i++)
